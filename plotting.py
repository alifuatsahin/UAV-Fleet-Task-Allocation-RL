...
df3_subset['rolling_mean'] = df3_subset['transformed_score'].rolling(window=1000).mean()
df3_subset['rolling_std'] = df3_subset['transformed_score'].rolling(window=1000).std()
...
print(f"File 3 - Rolling Mean: {df3_subset['rolling_mean'].iloc[-1]}, Rolling Variance: {df3_subset['rolling_std'].iloc[-1]**2}")

# Plotting the data
plt.figure(figsize=(10, 6))

# Plot rolling mean and variance as transparent area for File 1
plt.plot(df1_subset['episode number'], df1_subset['rolling_mean'], label='Dirichlet-0.01')
...
plt.plot(df2_subset['episode number'], df2_subset['rolling_mean'], label='Gaussian-Softmax-0.01')
plt.fill_between(df2_subset['episode number'],
                 df2_subset['rolling_mean'] - df2_subset['rolling_std'],
                 df2_subset['rolling_mean'] + df2_subset['rolling_std'],
                 color='orange', alpha=0.1)

plt.plot(df3_subset['episode number'], df3_subset['rolling_mean'], label='Gaussian-Softmax-0.001')
plt.fill_between(df3_subset['episode number'],
                 df3_subset['rolling_mean'] - df3_subset['rolling_std'],
                 df3_subset['rolling_mean'] + df3_subset['rolling_std'],
