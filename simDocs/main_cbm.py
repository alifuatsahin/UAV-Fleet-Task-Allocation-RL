...
def main(show_graphs=True):
    ...
    for i in range(NUMBER_OF_UAVS):
        ...
        #         ax.plot(uav_complete.iloc[:, 4 + j], color=bearing_col_list[j], alpha=0.3, label=f"Hover {j} bearing health")
        #     #
        #     coil_col_list = ['C4', 'C5', 'C6', 'C7']
        #     for j in range(len(uav.hover_coil_health)):
        #         ax.plot(uav_complete.iloc[:, 12 + j], color=coil_col_list[j], alpha=0.3, label=f"Hover {j} coil health")
        #
        #     ax.plot(uav_complete.iloc[:, 20], color='C8', alpha=0.3, label="Pusher bearing health")
        #     ax.plot(uav_complete.iloc[:, 22], color='C9', alpha=0.3, label="Pusher coil health")
        #     ax.plot(uav_complete.iloc[:, 24], alpha=0.2, label="Battery capacity")
        #     plt.plot(uav_complete.iloc[:, 40], 'k', label="Health Index")
