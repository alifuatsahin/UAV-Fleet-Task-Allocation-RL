...
class Agent:
    def __init__(self, env,
        ...
        if auto_entropy:
            self.target_entropy =  -6.423 * np.exp((0.12 + 0.045)*env.action_space.shape[0]) + 8.406  # -0.98 * np.log(1/env.action_space.shape[0]) -th.prod(th.Tensor(env.action_space.shape[0]).to(self.device)).item()
